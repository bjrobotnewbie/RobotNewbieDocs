...
| 项目                                                                                    | 核心特性                       | 性能优势       | 适用场景     |
| ------------------------------------------------------------------------------------- | -------------------------- | ---------- | -------- |
| [vLLM](deployment/vllm/README)                                          | PagedAttention, 连续批处理      | 吞吐量提升 2-4x | 高吞吐量云端推理 |
| [TensorRT-LLM](deployment/tensorrt-llm/README)                                | NVIDIA 官方, AutoDeploy, FP8 | 极致延迟优化     | 数据中心生产环境 |
| [LMDeploy](deployment/lmdeploy/README)                                      | TurboMind 引擎, AWQ 4bit     | 中文模型优化     | 国内生态友好   |
| [Text Generation Inference](deployment/text-generation-inference/README) | Hugging Face 官方, 流式输出      | 生产级稳定性     | 企业级服务部署  |
| [LLama-CPP](deployment/llama_cpp/README) | 本地推理, 零 Python 依赖 | 嵌入式设备 / Jetson / Raspberry Pi | 资源受限环境, 本地隐私推理 |
...
| 项目                                                 | 核心特性                           | 典型设备                          | 适用场景           |
| -------------------------------------------------- | ------------------------------ | ----------------------------- | -------------- |
| [Ollama](deployment/ollama/README)         | 单二进制, 一键启动                     | x86/ARM 桌面                    | 快速原型验证         |
| [llama.cpp](deployment/llama_cpp/README) | C/C++ 实现, GGUF 格式, 零 Python 依赖 | 嵌入式设备 / Jetson / Raspberry Pi | 资源受限环境, 本地隐私推理 |
| [Candle](deployment/candle/README)    | Rust 实现, 零 Python 依赖           | 嵌入式设备                         | 资源受限环境         |
...
| 项目                                                       | 核心特性                   | 硬件支持         | 适用场景     |
| -------------------------------------------------------- | ---------------------- | ------------ | -------- |
| [ONNX Runtime](deployment/onnx-runtime/README) | 跨平台, 算子融合, 多后端         | CPU/GPU/NPU  | 通用模型部署   |
| [OpenVINO](deployment/openvino/README)  | Intel 优化, NNCF 量化, 工具链 | CPU/iGPU/VPU | 工业机器人    |
| [Ray Serve](deployment/ray-serve/README)          | 分布式, 自动扩缩容, 流量灰度       | 集群节点         | 大规模机器人集群 |
